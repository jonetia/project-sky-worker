...
function database({ lookupError = false, lookupFailures = 0, truncatedObjects = false, pageCap = 1, insertError = false } = {}) {
  const identifiers = [], elements = new Map(), calls = [];
  let sequence = 0, remainingLookupFailures = lookupFailures;
  const client = createClient("https://example.supabase.co", "test-key", { auth: { persistSession: false }, global: { fetch: async (input, options = {}) => {
    ...
    if (table === "object_identifiers" && !rows) {
      if (lookupError || remainingLookupFailures-- > 0) return json({ code: "57014", message: "statement timeout" }, 500);
...
test("transient identifier timeouts are retried safely", async () => {
  const db = database({ lookupFailures: 2 });
  const result = await ingestGroup(db.client, "active", options([record()]));
  assert.equal(result.identifiersInserted, 2);
  assert.equal(result.elementsWritten, 1);
  assert.equal(db.calls.filter(c => c.table === "object_identifiers" && !c.rows).length, 3);
});
...
test("identifier reads are bounded to 25 objects per request", async () => {
  const db = database({ pageCap: 500 });
  await ingestGroup(db.client, "active", options(Array.from({ length: 60 }, (_, i) => record(100001 + i))));
  const sizes = db.calls
    .filter(c => c.table === "object_identifiers" && !c.rows)
    .map(c => c.url.searchParams.get("object_id").split(",").length);
